...
class MdlDeveloper(CFG):
	...
	def _DefineModels(self):
		...
		self.Mdl_Master = {
			...
			'LGBM4C': LGBMC(**{
				'device': "gpu" if self.gpu_switch == "ON" else "cpu", 'objective': 'binary',
				'boosting_type': 'gbdt',
				'metric': "auc",
				'random_state': self.state,
...
if CFG.ML == "Y":
	...
	for target in CFG.targets:
		md = MdlDeveloper(Xtrain, ytrain[target], ytrain[target], Xtest, sel_cols=sel_cols, cat_cols=cat_ftre,
						  enc_cols=[]);
